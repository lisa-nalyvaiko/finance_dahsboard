...
def update_revenue_graph(xaxis_column_name):
    grouped_df = revenue.groupby(xaxis_column_name, as_index=False).agg({'Total price': 'sum'}).sort_values(
...
def update_dev_stats_graph(dev_criterium, dev_units):
    d = dev_stats
    if dev_units == "Hours":
        df = d.groupby(dev_criterium, as_index=False)\
                .agg({'Unbilled': 'sum', 'Billed hours': 'sum'})\
                .sort_values(by='Billed hours',ascending=False)
        figure = go.Figure(data=[
            go.Bar(name="Unbilled hours", x=df[dev_criterium], y=df['Unbilled'], marker_color='#c2acbf'),
            go.Bar(name='Billed hours', x=df[dev_criterium], y=df['Billed hours'], marker_color='#a51140')
        ], )
        figure.update_layout(barmode='group')
        return figure
    df = d.groupby(dev_criterium, as_index=False)\
            .agg({'Cost': 'sum', 'Revenue': 'sum', 'Profitability, $': 'sum'})\
            .sort_values(by='Profitability, $', ascending=False)
    figure = go.Figure(data=[
        go.Bar(name="Developers cost", x=df[dev_criterium], y=df['Cost'], marker_color='#c2acbf'),
        go.Bar(name='Revenue generated by developer', x=df[dev_criterium], y=df['Revenue'], marker_color='#a51140')
    ])
    figure.update_layout(barmode='group')
    return figure
